...
test_that("can get queued status", {
  expect_status_from_queue("PENDING", list(
    runStatus = "queued",
    runSuccess = NULL,
    done = FALSE,
    runErrors = NULL,
...
test_that("can get running status", {
  expect_status_from_queue("RUNNING", list(
    runStatus = "running",
    runSuccess = NULL,
    done = FALSE,
    runErrors = NULL,
...
test_that("can get complete status", {
  expect_status_from_queue("COMPLETE", list(
    runStatus = "complete",
    runSuccess = TRUE,
    done = TRUE,
    runErrors = NULL,
...
test_that("can get failed status", {
  ...
  mockery::stub(queue$get_run_status,
                "rrq::rrq_task_result",
                mock_rrq_task_result)

  status <- queue$get_run_status("12345")

  expect_identical(status, list(
    runStatus = "failed",
    runSuccess = FALSE,
    done = TRUE,
...
test_that("can get run results", {
  ...
  mockery::stub(queue$get_run_results,
                "rrq::rrq_task_result",
                mock_rrq_task_result)
